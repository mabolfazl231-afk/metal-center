...
import { TrendingUp, FileText, Calculator, Factory as FactoryIcon, CircleDot, Layers, Square, RectangleHorizontal, Box } from "lucide-react";
...
const CATEGORY_ICONS: Record<string, any> = {
  "میلگرد": CircleDot,
  "تیرآهن": Layers,
  "ورق": RectangleHorizontal,
  "پروفیل": Box,
  "نبشی و ناودانی": Square,
};
...
export default function HomePage() {
  return (
    <>
      ...
      {/* Quick actions */}
      <section className="bg-steel px-6 py-12 dark:bg-white/[0.02]">
      ...
      {/* Categories */}
      <section className="bg-white px-6 py-14 dark:bg-canvas-dark">
        <div className="mx-auto max-w-[1240px]">
        ...
        <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-5">
          {categories.map((c) => {
            const Icon = CATEGORY_ICONS[c.name] ?? Box;
            return (
              <Link
                key={c.name}
                href="/products"
                className="group rounded-card border border-border-light bg-white p-5 text-center shadow-soft transition-all hover:-translate-y-1 hover:border-blue hover:shadow-lifted dark:border-border-dark dark:bg-surface-dark"
              >
                <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue/10 transition-colors group-hover:bg-blue">
                  <Icon size={22} className="text-blue transition-colors group-hover:text-white" />
                </div>
      ...
      {/* Featured products */}
      <section className="bg-steel px-6 py-14 dark:bg-white/[0.02]">
        <div className="mx-auto max-w-[1240px]">
        <div className="mb-8 flex items-end justify-between">
      ...
      {/* CTA */}
      <section className="bg-steel px-6 py-16 dark:bg-white/[0.02]">
        <div className="mx-auto max-w-[1240px] rounded-card bg-blue px-8 py-12 text-center text-white">
