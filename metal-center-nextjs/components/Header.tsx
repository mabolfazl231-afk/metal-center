...
export default function Header() {
  ...
  return (
    ...
    >
      {/* ===== Desktop header ===== */}
      <div
        className={`mx-auto hidden max-w-[1240px] grid-cols-[1fr_auto_1fr] items-center gap-5 px-6 transition-all duration-300 lg:grid ${
      ...
      {/* نوار ناوبری اصلی زیر لوگو (دسکتاپ) */}
      <nav className="hidden justify-center gap-1 border-t border-border-light/60 py-1.5 dark:border-border-dark/60 lg:flex">
        {NAV_LINKS.map((link) => (
          <div key={link.href} onMouseEnter={() => setOpenMega(link.mega ?? null)} className="relative">
            <Link
              href={link.href}
              className="rounded-[10px] px-4 py-2 text-[14px] font-medium text-ink/65 transition-all duration-hover hover:bg-blue/10 hover:text-blue dark:text-white/65 dark:hover:bg-blue/20 dark:hover:text-white"
            >
              {link.label}
            </Link>
          </div>
      ...
      {/* ===== Mobile header ===== */}
      <div className="flex h-[64px] items-center justify-between gap-2 px-3 lg:hidden">
        <button
          type="button"
        ...
        <Link href="/" className="flex min-w-0 flex-1 items-center justify-center">
          <Image src="/logo.png" alt="متال سنتر" width={220} height={78} className="h-14 w-auto object-contain" priority />
        </Link>
